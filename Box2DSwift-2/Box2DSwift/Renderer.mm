...
@interface Renderer () {
    ...
    GLuint brickVertexArray,brickCPUVertexArray, ballVertexArray;   // vertex arrays for brick and ball
...
- (void)update
{
    ...
    b2Vec2 *theBrickCPU = (((*objPosList).find("brickCPU") == (*objPosList).end()) ? nullptr : &(*objPosList)["brickCPU"]);
    ...
    {
        // Set up VAO/VBO for brick
        glGenVertexArrays(1, &brickCPUVertexArray);
        glBindVertexArray(brickCPUVertexArray);
        GLuint vertexBuffers[2];
        glGenBuffers(2, vertexBuffers);
        ...
        int k = 0;
        numBrickVerts = 0;
        vertPos[k++] = theBrickCPU->x - BRICK_CPU_WIDTH/2;
        vertPos[k++] = theBrickCPU->y + BRICK_CPU_HEIGHT/2;
        vertPos[k++] = 10;  // z-value is always set to same value since 2D
        numBrickVerts++;
        vertPos[k++] = theBrickCPU->x + BRICK_CPU_WIDTH/2;
        vertPos[k++] = theBrickCPU->y + BRICK_CPU_HEIGHT/2;
        ...
        numBrickVerts++;
        vertPos[k++] = theBrickCPU->x - BRICK_CPU_WIDTH/2;
        vertPos[k++] = theBrickCPU->y + BRICK_HEIGHT/2;
        vertPos[k++] = 10;
        numBrickVerts++;
        vertPos[k++] = theBrickCPU->x + BRICK_CPU_WIDTH/2;
        vertPos[k++] = theBrickCPU->y - BRICK_CPU_HEIGHT/2;
...
- (void)draw:(CGRect)drawRect;
{
    ...
    glBindVertexArray(brickCPUVertexArray);
    if (theBrickCPU && numBrickVerts > 0)
        glDrawArrays(GL_TRIANGLES, 0, numBrickVerts);
}
